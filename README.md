...
npm install jsdom --no-save && npm test   # smoke 350 + sync 20 + transfer 23 + chart 44 + header 22
```
...
| **Trợ lý chat & quét bill** | Nút nổi 💬 ở góc dưới mở một ngăn chat: gõ *"cà phê 35k"*, *"nhận lương 15tr hôm qua"* hoặc gửi/dán ảnh hoá đơn, bot bóc ra số tiền (`35k`, `1.2tr`, `1.2m`, `50.000`), ngày và loại thu/chi rồi dựng **thẻ xác nhận** kèm hai nút *Tự động lưu* / *Tùy chỉnh thêm*. Danh mục thì nó **tự học từ lịch sử của chính bạn**: quét ghi chú các giao dịch cũ, nhớ cụm từ nào từng đi với danh mục nào (không dấu cũng khớp), và nhớ cả danh mục con. Không nhận ra thì gán *Khác* và nói thẳng "Chưa nhận diện được, bấm để đổi" — sửa một lần là lần sau nó nhớ. Bot **không tự ghi vào sổ**, phải bấm mới lưu. Đọc chữ trên ảnh cần tải công cụ OCR (~2MB) lần đầu và có hỏi trước; không tải hoặc mất mạng thì vẫn nhập tay số tiền được. |
...
| **Vuốt lùi / nút Back** | Mỗi bước điều hướng — đổi tab, vào màn hình con, mở modal, mở bàn phím số hay ngăn chat — là một entry trong `history`, nên nút Back cứng của Android và cú vuốt mép trên iOS/trình duyệt lùi **đúng một bước** thay vì thoát app. Quay lại một tab thì bộ lọc của nó cũng trở về đúng như lúc rời đi. Đóng hộp xác nhận bằng Back được hiểu là *Huỷ*. Bản Android bắt luôn nút Back cứng và cú vuốt predictive back: còn bước thì lùi, hết bước mà còn cửa sổ đang mở thì đóng cửa sổ, chỉ khi ở màn gốc và không còn gì mở mới thoát app. Lưu giao dịch xong thì bước đó coi như khép lại — Back không mở lại cái form vừa gửi. URL không đổi: hash là của Supabase Auth. |
| **Ghi nhanh (Direct-to-Keypad)** | Nút **+** mở thẳng bàn phím số thay vì form: gõ số, gõ ghi chú, bấm **Lưu**. Ghi chú gõ tới đâu thì **ví và danh mục tự nhảy theo lịch sử** tới đó ("xăng" → Di chuyển + đúng cái ví bạn hay trả tiền xăng) — chọn tay thì lựa chọn của bạn thắng. Công tắc Chi/Thu ở đầu màn. Cần ngày khác hôm nay, sự kiện hay danh mục con thì có "Thêm chi tiết ›" mở form đầy đủ, mang theo số tiền và ghi chú đang gõ. |
| **Giao dịch gần đây** | 5 bản ghi mới nhất ngay dưới lưới Tiện ích ở Tổng quan, dùng đúng hàng giao dịch của tab Giao dịch (chạm để mở chi tiết). Chỉ khoản **đã ghi nhận** — khoản dự kiến có khối "Sắp đến hạn" riêng, để một khoản chi chưa xảy ra không chiếm đầu danh sách "gần đây". "Xem tất cả ›" mở toàn bộ sổ, không mang theo bộ lọc nào. |
| **Hỏi trợ lý về số liệu** | Ngoài việc ghi, trợ lý trả lời được: *"tháng này chi nhiều nhất vào đâu?"*, *"ăn uống tháng trước bao nhiêu?"*, *"còn bao nhiêu tiền trong Ngân hàng?"*. Con số tính từ chính sổ của bạn bằng đúng hàm mà tab Báo cáo dùng, kèm một đường dẫn **sang màn hình đã lọc sẵn** đúng kỳ / ví / danh mục — hỏi về một khoản cụ thể thì bấm vào là mở thẳng chi tiết bản ghi đó. |
| **Nhắn là xong: chuyển ví & định kỳ** | Trợ lý hiểu ba loại câu: *"chuyển 2tr từ Ngân hàng sang Tiền mặt"* (chuyển ví), *"tiền nhà 4tr hàng tháng ngày 5"* (tạo lịch định kỳ), và thu/chi thường. Thiếu thông tin thì **hỏi lại bằng nút bấm** — thiếu ví đích thì hiện dãy chip ví, thiếu chu kỳ thì hiện [Hàng tuần] [Hàng tháng] [Hàng năm] — chứ không đoán hộ chỗ tiền sẽ nằm. Đủ thì ra thẻ xác nhận kèm **[✓ Xác nhận]** và **[Chỉnh sửa ➔]** (mở form thật, nơi duy nhất có phí chuyển và tỷ giá khi hai ví khác tiền tệ). |
| **Đọc hoá đơn in nhiệt** | Bill quán ăn / cửa hàng tiện lợi / siêu thị có rất nhiều dòng tiền, và trợ lý biết dòng nào là **tổng phải trả**: nó đi theo nhãn ("Tổng cộng", "Cần trả", "T.Tiền") và **loại thẳng** "Tiền khách trả" với "Tiền thối lại" — hai dòng mà cách "lấy số lớn nhất" hay "lấy số cuối" luôn đọc nhầm. Ghi chú lấy tên quán, danh mục suy từ tên cửa hàng (quán ăn, cây xăng, siêu thị, nhà thuốc…) nhưng **lịch sử của bạn luôn thắng** bảng đoán sẵn. Chịu được cả khi OCR đọc "95.000" thành "95 000". |
| **Quét không ra thì nói rõ vì sao** | Ba tình huống, ba câu trả lời: *không đọc được chữ nào* (ảnh mờ / chưa tải công cụ OCR), *đọc được chữ nhưng không thấy dòng số tiền* (ảnh cắt mất phần tổng), và *số tiền đáng ngờ* (số to bất thường không nằm cạnh nhãn nào — có thể là mã hoá đơn). Hai trường hợp đầu **không tạo giao dịch lỗi**: trợ lý nêu lại thứ đã đọc được (tên quán, ngày) và cho ba đường đi tiếp — nhập tay số tiền, chọn ảnh khác, hoặc mở form đầy đủ. |
| **Đọc biên lai Techcombank / MoMo** | Ảnh biên lai chuyển tiền được bóc theo cấu trúc: nhà phát hành, số tiền (`2,000,000 VND`, `-55.000đ`), nội dung chuyển khoản, ngày — rồi **tự chọn ví tương ứng trong sổ** (Techcombank → ví ngân hàng, MoMo → ví điện tử) và gán danh mục từ lịch sử. Toàn bộ khớp trên văn bản đã bỏ dấu, vì OCR đọc dấu tiếng Việt rất tệ. Không phải biên lai thì về luật hoá đơn thường (dòng "tổng cộng"). |
| **Trung tâm thông báo** | Cảnh báo ngân sách, nợ đến hạn và thẻ tới ngày trả không còn là banner chiếm chỗ trên Trang chủ — chúng vào quả chuông trên app bar, có **chấm đỏ khi chưa đọc**. Bấm một thông báo là nó được đánh dấu đã đọc và app mở thẳng màn hình xử lý được nó (ngân sách theo danh mục → Giao dịch đã lọc, ngân sách tổng → Báo cáo). Trạng thái đã đọc **đồng bộ đa thiết bị**: đọc trên điện thoại thì máy tính cũng thôi nhắc. Mốc 80% và 100% là hai tin riêng. |
| **Lịch sử hội thoại & Tạo lại** | Trợ lý giữ **100 bản ghi gần nhất**, cuộn lại xem được cả những giao dịch đã ghi. Mỗi giao dịch cũ có nút **🔄 Tạo lại** — dựng lại y khoản đó với ngày hôm nay, chỉ cần xác nhận; ví hoặc danh mục đã bị xoá thì nó nói và chọn tạm cái khác. Lịch sử lưu riêng theo từng tài khoản trên máy, không đẩy lên cloud. |
| **Hỏi lại khi thiếu thông tin** | Nhắn *"Vừa mua áo mới"* (không có số tiền) thì trợ lý **không tạo giao dịch lỗi** — nó hỏi lại *"Khoản này hết bao nhiêu?"* và nhớ phần đã hiểu. Trả lời *"200k"* là ra thẻ xác nhận hoàn chỉnh với ghi chú của câu đầu. Ý định dở dang có hạn 10 phút, và một câu mới có số tiền được hiểu là giao dịch mới chứ không dính vào câu cũ. |
| **Vuốt ngang đổi tab** | Vuốt trái/phải trên nội dung để đi giữa **Tổng quan · Giao dịch · Báo cáo · Cài đặt**, mỗi cú vuốt là một bước lịch sử nên nút Back lùi đúng một tab. Không cướp cử chỉ của thứ khác: thanh ví và băng chip vẫn cuộn ngang bình thường, 24px sát mép trái nhường cho cử chỉ Back của hệ điều hành, vuốt chéo hay quá ngắn thì bỏ qua, và khi đang mở modal / bàn phím nhập tiền / ngăn chat thì không đổi tab sau lưng chúng. |
...
| **Giao diện** | Ngôn ngữ thiết kế **VietinBank iPay**: app bar gradient xanh gọn một hàng (~70px), thẻ tổng tài sản trắng bo 16px đè lên app bar với nút con mắt ngay cạnh con số, ví xếp thành **thanh cuộn ngang** luôn hở một phần thẻ kế tiếp để thấy là cuộn được, nền `#F4F7FA`. Thứ tự trang chủ: **Tổng tài sản → Giao dịch gần đây → Tiện ích → Ví → Sắp đến hạn → Chi tiêu & ngân sách**; khối Sắp đến hạn **tự ẩn khi không có gì để nhắc**, còn hạn mức ngân sách hiện ngay trên hàng danh mục tương ứng (`2.602.000 đ / 3.000.000 đ` · `Còn 398.000 đ`) thay vì một khối riêng. Lưới **Tiện ích 5 ô**, icon trong khung gradient nhạt có inner shadow. Bottom nav kính mờ (`backdrop-filter`), tab active xanh kèm vạch đỏ `#ED1C24`; nút **+** là FAB gradient xanh–đỏ có glow thở nhẹ. Ripple + rung nhẹ khi chạm. Mobile-first 480px, Dark/Light/Tự động. |
...
- **Quét bill cần mạng ở lần đầu.** Engine OCR (tesseract.js) được tải theo yêu cầu từ CDN chứ không đóng gói sẵn — vendor ~2MB JS cộng wasm cộng gói tiếng Việt sẽ nhân đôi kích thước APK. Mọi phần còn lại của trợ lý (bóc số tiền, ngày, gán danh mục) chạy hoàn toàn offline.
- Trợ lý đọc hoá đơn bằng OCR chung, **không** huấn luyện riêng cho mẫu hoá đơn Việt Nam: ảnh mờ, chụp nghiêng hoặc in kim mờ mực thì nên nhập tay. Thẻ xác nhận luôn cho sửa trước khi lưu, đúng vì lý do đó.
