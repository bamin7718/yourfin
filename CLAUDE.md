...
- **Biên lai ngân hàng / ví điện tử** (`parseBankReceiptOCR()`): nhận nhà phát hành qua `BANK_SIGNS` (Techcombank, MoMo, ZaloPay, VCB, VietinBank, BIDV, MB, ACB, VPBank), bóc số tiền theo **nhóm nghìn có dấu phân cách** (`2,000,000` / `100.000` — không thể là số tài khoản), rồi nội dung chuyển khoản, rồi map sang ví trong sổ.
  ...
  - **Hoá đơn in nhiệt** (`parseThermalReceiptOCR()`) là tầng giữa: `parseBankReceiptOCR()` không thấy nhà phát hành nào thì rơi vào đây, rồi mới tới luật chung. Hai đặc thù quyết định cách bóc:
    - **Con số lớn nhất KHÔNG phải tổng phải trả** — khách đưa 200.000 cho hoá đơn 95.000. Con số **cuối cùng** cũng không (sau dòng tổng còn "Tiền khách trả", "Tiền thối lại", rồi số điện thoại ở chân bill). Nên: đi theo **nhãn** (`THERMAL_TOTAL_RE`, nhãn ở dòng SAU thắng — bill in "Tổng tiền hàng" → giảm giá → "Tổng cộng"), và có `THERMAL_REJECT_RE` loại thẳng những dòng tiền khách/tiền thối. Không nhãn nào đọc được thì mới lấy số lớn nhất, **trừ** các dòng bị loại.
    - OCR in nhiệt hay đọc dấu ngăn nghìn thành **dấu cách** ("95 000"). `thermalJoinSpacedNumbers()` gộp lại, nhưng chỉ khi cụm đó đứng riêng — không thì hai số cạnh nhau bị dán thành một. **Không dùng lookbehind**: một regex literal có lookbehind là lỗi *cú pháp* trên Safari cũ, và lỗi cú pháp thì cả `app.js` không nạp được, không phải chỉ hàm đó.
  - **Thang tin cậy khi chọn danh mục cho bill nhiệt** (6 bậc, mỗi bậc có một ca thật): cụm từ học từ lịch sử (`fromPhrase`) → từ khoá cửa hàng khớp trong **tên quán** → khớp bất kỳ từ lịch sử → từ khoá cửa hàng ở bất kỳ đâu → khớp theo tên danh mục → "Khác". Lý do bậc 1 dùng **cụm từ** chứ không phải ngưỡng điểm: điểm cộng dồn theo số lần xuất hiện, nên một chữ tầm thường lặp ba lần cũng vượt mọi ngưỡng. Ca thật: bỏ dấu xong **"QUÁN cơm tấm" dùng chung chữ 'quan' với "QUẦN áo"**, và dữ liệu mẫu có đúng ghi chú đó.
  - **`validateOCRResult()` — ba kiểu thất bại, ba câu trả lời khác nhau.** Gộp cả ba vào "không đọc được ảnh" là đẩy người dùng vào chỗ không biết làm gì tiếp (chụp lại? bật mạng? gõ tay?): `no-text` (không đọc được chữ nào — ảnh mờ hoặc engine chưa tải), `no-amount` (có chữ nhưng không thấy dòng tổng), `suspicious` (có số nhưng **không do nhãn nào chỉ ra** và ≥ 500 triệu — rất có thể là mã hoá đơn hoặc số điện thoại). Hai kiểu đầu **không dựng thẻ xác nhận** mà gọi `showOCRErrorFallbackState()`: một thẻ thiếu số tiền thì nút "Tự động lưu" trên đó chỉ toast rồi đứng im. Trạng thái đó nêu lại **thứ đã đọc được** (tên quán, ngày) và cho ba đường đi tiếp: nhập tay số tiền · chọn ảnh khác · mở form đầy đủ.
  - Thẻ xác nhận **hiện cả ghi chú** và cho sửa tại chỗ (`chatEditNote()`): ghi chú đi vào sổ **và** vào ma trận từ khoá, nên một chuỗi OCR đọc sai mà lưu luôn thì nó dạy sai cho cả lần sau.
